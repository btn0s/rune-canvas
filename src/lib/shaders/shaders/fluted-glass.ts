...
// Fluted Glass shader - requires image input
// language=GLSL
...
function paramsToUniforms(params: ShaderParams): ShaderUniforms {
  ...
  return {
    u_colorBack: colorToVec4((params.colorBack as string) || "#00000000"),
    u_colorShadow: colorToVec4((params.colorShadow as string) || "#000000"),
    u_colorHighlight: colorToVec4((params.colorHighlight as string) || "#ffffff"),
    u_image: image,
    u_imageAspectRatio: imageAspectRatio,
    u_shadows: (params.shadows as number) ?? 0.25,
    u_size: (params.size as number) ?? 0.5,
    u_angle: (params.angle as number) ?? 0,
    u_distortionShape: distortionShapeMap[(params.distortionShape as string) || "prism"] ?? 1,
    u_highlights: (params.highlights as number) ?? 0.1,
    u_shape: shapeMap[(params.shape as string) || "lines"] ?? 1,
    u_distortion: (params.distortion as number) ?? 0.5,
    u_shift: (params.shift as number) ?? 0,
    u_blur: (params.blur as number) ?? 0,
    u_edges: (params.edges as number) ?? 0.25,
    u_marginLeft: (params.marginLeft as number) ?? (params.margin as number) ?? 0,
    u_marginRight: (params.marginRight as number) ?? (params.margin as number) ?? 0,
    u_marginTop: (params.marginTop as number) ?? (params.margin as number) ?? 0,
    u_marginBottom: (params.marginBottom as number) ?? (params.margin as number) ?? 0,
    u_stretch: (params.stretch as number) ?? 0,
    u_grainMixer: (params.grainMixer as number) ?? 0,
    u_grainOverlay: (params.grainOverlay as number) ?? 0,
  };
}

export const flutedGlassShader: ShaderDefinition = {
  id: "fluted-glass",
  name: "Fluted Glass",
  description: "Glass distortion effect with ribbed patterns",
